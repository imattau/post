...
import { graph, putNode, deleteNode, getNodesOrdered, flushGraph, messageSearchText } from "@/lib/db/poly";
...
export const useMessagesStore = create<MessagesState>()(immer((set, get) => ({
  ...
  async loadFromCache() {
    const messages = await getNodesOrdered<Message>('message', 'createdAt');
  ...
  async markRead(id: string) {
    ...
    await putNode('message', id, updated as any, messageSearchText(updated as any));
  ...
  ingestFromRelay: async (message: Message) => {
    ...
    await putNode('message', message.id, message as any, messageSearchText(message as any));
    ...
    const check = () => {
      ...
      if (Object.keys(updates).length > 0) {
        ...
        for (const msg of Object.values(updates)) {
          putNode('message', msg.id, msg as any, messageSearchText(msg as any));
        }
      }
