...
export default function IdentityDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Dialog.Root open onOpenChange={(open) => { if (!open) onClose(); }}>
      <Dialog.Backdrop className="fixed inset-0 z-40" style={{ backgroundColor: "rgba(5,7,11,0.44)" }} />
      <Dialog.Portal>
        <Dialog.Popup className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 outline-none">
