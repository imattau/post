import type { MockAttachment } from "@/lib/mock/threads";
...
export default function AttachmentCard({ attachment }: { attachment: MockAttachment }) {
  return (
    <div className="flex items-center gap-3 h-[88px] px-3 border border-border rounded-pill bg-sidebar w-[274px]">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[12px] font-semibold text-text-primary truncate">{attachment.fileName}</p>
        <p className="text-[10px] text-text-tertiary mt-0.5">
          {formatSize(attachment.sizeBytes)} · Blossom{attachment.encrypted ? " / encrypted" : ""}
        </p>
